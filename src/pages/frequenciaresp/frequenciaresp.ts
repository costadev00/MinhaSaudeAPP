...
export class FrequenciarespPage {
 ...
 calcularfreqresp(){
	...
	if (freq<12)
	{
		toast.setMessage("CUIDADO! Você pode estar com bradipneia (frequência muito baixa). Procure um médico " + this.nome.value);
		toast.present();
	...
	{
		toast.setMessage("ATENÇÃO! Você pode estar com taquipneia (frequência muito alta). Procure um médico, " + this.nome.value);
		toast.present();
	}
